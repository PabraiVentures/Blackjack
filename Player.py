...
class Player:
	def __init__(self,id):
		...
		self.money=1000
	def getPlayerDecisionsForHand(self,handIdx): #gets one decision for one hand
		goodInput = False
		while(goodInput == False):
			
			if(len(self.hands[handIdx]) == 2):
				if((self.hands[handIdx][0].value() == self.hands[handIdx][1].value()) &&(self.bets[handIdx] <= self.money ) ):
